...
const Men = () => {
  ...
  const handleEditSubmit = () => {
    if (selectedProductId) {
      axios.put(`http://localhost:3000/men/${selectedProductId}`, editedData)
        .then((res) => {
  ...
  const handleDeleteClick = (productId) => {
    ...
    if (confirmDelete) {
      axios
        .delete(`http://localhost:3000/men/${productId}`)
        .then((res) => {
            ...
            <div className='row'>
              {user?.map((pro) => (
                <div className='col' key={pro.id}>
                  <div className='card mb-3 mt-3' style={{width:"250px", height:"400px"}}>
                    ...
                    <div className="text-center">
                      ...
                      <p style={{color:"red"}}>Size: {pro.quantity}</p> {/* Fix Quantity */}
                    </div>
