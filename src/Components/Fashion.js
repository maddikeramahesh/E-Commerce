...
const Fashion = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/fashion')
      .then((response) => {
  ...
  const handleEditSubmit = () => {
    if (selectedProductId) {
      axios.put(`http://localhost:3000/fashion/${selectedProductId}`, editedData)
        .then((res) => {
  ...
  const handleDeleteClick = (productId) => {
    ...
    if (confirmDelete) {
      axios
        .delete(`http://localhost:3000/fashion/${productId}`)
        .then((res) => {
            ...
            <div className='row'>
              {user?.map((pro) => (
                <div className='col' key={pro.id}>
                  <div className='card mb-3 mt-3' style={{width:"250px", height:"350px"}}>
